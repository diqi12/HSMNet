...
from .dataset import CoverStegoDataset
...
def build_train_loader(cover_dir, stego_dir, batch_size=32, num_workers=0,train_f=True):
    transform = torchvision.transforms.Compose([
        RandomRot(),
        RandomFlip(),
        ToTensor(),
    ])
    dataset = CoverStegoDataset(cover_dir, stego_dir, transform,train_f)
...
def build_val_loader(cover_dir, stego_dir, batch_size=32, num_workers=0,train_f=False):
    transform = torchvision.transforms.Compose([
        ToTensor(),
    ])
    dataset = CoverStegoDataset(cover_dir, stego_dir, transform,train_f)
